...
int inCTOS_SettleAHost(void)
{
	...
	for(inNum =0 ;inNum < inNumOfMit; inNum++)
	{
		...
		vdDebug_LogPrintf("settel a host %d - %d", srTransRec.MITid, strHDT.inHostIndex);
...
int inCTOS_SettleAllHosts(void)
{
	...
	for (i = 0; i < 5; i++)
	{
		if(inHDTRead(inNum) == d_OK)
		{
			...
			vdDebug_LogPrintf("test lang host index - %d",strHDT.inHostIndex);
...
int inCTOS_SettleAllHostsWithPHQR(void)
{
    int inRet = d_NO;
	int inNumOfHost = 0,inNum;
	char szBcd[INVOICE_BCD_SIZE+1];
...
int inCTOS_SettleAllOperationWithPHQR(void)
{
    int inRet = d_NO;
	int inNumOfHost = 0,inNum;
	char szBcd[INVOICE_BCD_SIZE+1];
...
int inCTOS_BATCH_REVIEW_Process(int inType)
{
    int inRet;
...
int inCTOS_BATCH_REVIEW(void)
{
    ...
    inCTOS_BATCH_REVIEW_Process(BATCH_REVIEW);
...
int inCTOS_SETTLE_ALL_WALLET(void)
{
    int inRet = d_NO;
    ...
    inRet = inCTOS_SettleAllOperationWithPHQR();
