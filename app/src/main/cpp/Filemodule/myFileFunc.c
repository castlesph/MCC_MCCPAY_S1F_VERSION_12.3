...
int inMyFile_RecRead(void *p1)
{
    ...
    * If the specified file name does not exist , it will be created first. */
    ...
    {                                                                                       
        ...
        * Move backward from the end of the file.        */
        ...
        {                        
            ...
            {
                vdDebug_LogPrintf("[1. inMyFile_RecRead]---File Read Size[%d] != RecSize[%d]",ulReadOutSize, ulRecSize);
                CTOS_FileClose(*ulHandle);
                return ST_ERROR;    
            }                        
        }
        vdDebug_LogPrintf("2. [inMyFile_RecRead]---File Read Size[%d] == RecSize[%d]",ulReadOutSize, ulRecSize);                                                                                                                                                                                                                                       
        /* Close the opened file */
        if(strFile->fCloseFileNow)
        {
            vdDebug_LogPrintf("close file");
            if((inResult = CTOS_FileClose(*ulHandle)) != d_OK)
            {
	...
	vdDebug_LogPrintf("inMyFile_RecRead return");
    return ST_SUCCESS;    
}
...
int inCheckFileExist(const char *szFileName)
{
	...
	vdDebug_LogPrintf("inCheckFileExist, szFileName=[%s]", szFileName);
	
	pFile = fopen((char*)szFileName, "rb+");
	if (pFile == NULL)
		return d_NO;
	...
	vdDebug_LogPrintf("Size=[%d], szFileName=[%s], Exist = %s", inSize, szFileName, (inSize > 0 ? "Yes" : "No"));
	if (inSize>0)
		return d_OK;
	else
		return d_NO;
