...
USHORT usCTOSS_EMV_TxnPerform(void)
{
	...
	vdDebug_LogPrintf("saturn ::[*1]usCTOSS_EMV_TxnPerform::d_IPC_CMD_EMV_Txn_PerformEx::usResult[%d]", usResult);
    if(d_OK == usResult)
    {
        //status
        vdDebug_LogPrintf("saturn usOutLen=[%d],bOutBuf=[%s]",usOutLen,bOutBuf);
		if (memcmp(bOutBuf,SHARE_EMV_SUB_IPC,3) == 0)
		{
			...
			{	
				...
				vdDebug_LogPrintf("saturn ::usCTOSS_EMV_TxnPerform::PIN ENTRY::usResult[%d]", usResult);
				...
				memset(bOutBuf, 0x00, sizeof(bOutBuf));    
				usResult = inMultiAP_RunIPCCmdTypes("com.Source.SHARLS_EMV.SHARLS_EMV", d_IPC_CMD_EMV_Txn_PerformEx, bInBuf, usInLen, bOutBuf, &usOutLen);
				vdDebug_LogPrintf("::[*2]usCTOSS_EMV_TxnPerform::usResult[%d]", usResult);
				if(d_OK != usResult)
					return usResult;
				}
			}
	...
	vdDebug_LogPrintf("--usCTOSS_EMV_TxnPerform--end");
