...
USHORT inMultiAP_HandleIPC(BYTE *inbuf, USHORT inlen, BYTE *outbuf, USHORT *outlen)
{
	...
	vdDebug_LogPrintf("--inMultiAP_HandleIPC--");
	...
	vdDebug_LogPrintf("inbuf[0]=[%02x], [%d]", inbuf[0], inbuf[0]);
	...
	case d_IPC_CMD_QRPAY:		
    	vdDebug_LogPrintf("d_IPC_CMD_QRPAY");	
		inMCC_MultiSwitchApp(d_IPC_CMD_QRPAY);
		outbuf[len++] = d_IPC_CMD_QRPAY;
		outbuf[len] = 0x00;
		break;	
	case d_IPC_CMD_RETRIEVE:
		vdDebug_LogPrintf("d_IPC_CMD_RETRIEVE");	
		inMCC_MultiSwitchApp(d_IPC_CMD_RETRIEVE);
		outbuf[len++] = d_IPC_CMD_RETRIEVE;
		outbuf[len] = 0x00;
		break;
	...
	//inRet = inCTOS_SALE();
		...
		outbuf[len] = 0x00;
			break;
    //PHQR   
	case d_IPC_CMD_PHQR_SALE:		
    	vdDebug_LogPrintf("d_IPC_CMD_PHQR_SALE");	
		inMCC_MultiSwitchApp(d_IPC_CMD_PHQR_SALE);
		outbuf[len++] = d_IPC_CMD_PHQR_SALE;
		outbuf[len] = 0x00;
		break;	

	case d_IPC_CMD_PHQR_RETRIEVE:
		
    	vdDebug_LogPrintf("d_IPC_CMD_PHQR_RETRIEVE");	
		inMCC_MultiSwitchApp(d_IPC_CMD_PHQR_RETRIEVE);
		outbuf[len++] = d_IPC_CMD_PHQR_RETRIEVE;
		outbuf[len] = 0x00;
		break;	

    case d_IPC_CMD_PHQR_LOGIN:
    	vdDebug_LogPrintf("d_IPC_CMD_PHQR_LOGIN");	
		inMCC_MultiSwitchApp(d_IPC_CMD_PHQR_LOGIN);
		outbuf[len++] = d_IPC_CMD_PHQR_LOGIN;
		outbuf[len] = 0x00;
		break;
		
	case d_IPC_CMD_PHQR_SETTLE:
		inRet = inCTOS_SETTLE_PHQR();
		outbuf[len++] = d_IPC_CMD_PHQR_SETTLE;
		if (inRet == d_OK)
			outbuf[len++] = d_OK;
		else
			outbuf[len++] = d_FAIL;
	...
	case d_IPC_CMD_SETTLE_ALL_WALLET:
		inRet = inCTOS_SETTLE_ALL_WALLET();
		outbuf[len++] = d_IPC_CMD_SETTLE_ALL;
		if (inRet == d_OK)
			outbuf[len++] = d_OK;
		else
...
int inMultiAP_RunIPCCmdTypes(char *Appname, int IPC_EVENT_ID, BYTE *inbuf, USHORT inlen, BYTE *outbuf, USHORT *outlen)
{
	...
	vdDebug_LogPrintf("MCC PAY APP inMultiAP_RunIPCCmdTypes =[%d][%s]",pid, szAppname);
	if (pid <= 0)
	{
		inCTOS_ReForkSubAP(szAppname);
		memset(processID,0x00,sizeof(processID));
...
//		inIPC_KEY_SEND_Parent = pid;
		...
		{			
			...
			if (inMultiAP_IPCGetParent(IPC_OUT_BUF_STR, &ipc_len) == d_OK)
			{
				vdDebug_LogPrintf("inMultiAP_RunIPCCmdTypes inMultiAP_IPCGetParent ABCDE");
			
				ipc_len = wub_str_2_hex(IPC_OUT_BUF_STR, outbuf, ipc_len);
				*outlen = ipc_len;
				return d_OK;
			}
			else // fix for hang on processing during QRcode generation - for testing/ validation.
			{
			
				vdDebug_LogPrintf("inMultiAP_RunIPCCmdTypes inMultiAP_IPCGetParent d_NO");
				return d_NO;
			}
...
int inMultiAP_RunIPCCmdTypesEx(char *Appname, int IPC_EVENT_ID, BYTE *inbuf, USHORT inlen, BYTE *outbuf, USHORT *outlen)
{
	...
	vdDebug_LogPrintf("MCC PAY APP inMultiAP_RunIPCCmdTypesEx =[%d][%s],[%x]",pid, szAppname,IPC_EVENT_ID);
	if (pid <= 0)
	{
		inCTOS_ReForkSubAP(szAppname);
		memset(processID,0x00,sizeof(processID));
...
//		inIPC_KEY_SEND_Parent = pid;
		...
		while ( (wpid=waitpid(pid, &status, WNOHANG)) != pid )
		{
			...
			else // fix for hang on processing during QRcode generation - for testing/ validation.
			{
				...
				vdDebug_LogPrintf("inMultiAP_RunIPCCmdTypesEx inMultiAP_IPCGetParent d_NO");
				return d_NO;
			}
		}
	}
...
*/
    ...
    if(inECRType >=1) //wilcon ecr
    {
         ...
         if ((strlen(srTransRec.szRespCode) > 0) && (memcmp(srTransRec.szRespCode,"00",2) == 0) && (strlen(srTransRec.szECRRespCode)<=0))
         	{
    ...
    else //mercury ecr
    {
         ...
         srTransRec.szResponseText[40]=0x00;    
                ...
                put_env_int("ECRMCCPAYHOST",srTransRec.HDTid);
...
int inCTOS_MultiAPSaveData(int IPC_EVENT_ID)
{
	...
	inMultiAP_RunIPCCmdTypes("com.Source.S1_MCCPAY.MCCPAY",IPC_EVENT_ID,"",0, outbuf,&out_len);
...
int inMCC_MultiSwitchApp(int IPC_EVENT_ID)
{
   	int inRet = 0;
	int inTranType;
	...
	vdDebug_LogPrintf("inMCC_MultiSwitchApp start"); 
	...
	//fECRTxnFlg = get_env_int("ECRQR");


   vdDebug_LogPrintf("inMCC_MultiSwitchApp after inCTOSS_CheckMemoryStatus");  
   ...
   //vdCopyMenuFromMainApp();   
   vdDebug_LogPrintf("inMCC_MultiSwitchApp call inF1KeyEvent");  

   vdDebug_LogPrintf("testlang12 %d %x",IPC_EVENT_ID, IPC_EVENT_ID);

   
   vdDebug_LogPrintf("testlang12 %d ", inTranType);

   if (IPC_EVENT_ID == d_IPC_CMD_QRPAY)
   		vdDisplayTrxn(1, "mccpay.bmp");   		
   else if (IPC_EVENT_ID == d_IPC_CMD_RETRIEVE)
		vdDisplayTrxn(2, "mccpay.bmp");
   else if (IPC_EVENT_ID == d_IPC_CMD_SUMMARY_REPORT)
   		inF3KeyEvent();
   else if (IPC_EVENT_ID == d_IPC_CMD_REPRINT_LAST)
   		inCTOS_REPRINT_LAST();
   else if (IPC_EVENT_ID == d_IPC_CMD_REPRINT_ANY)
   		inCTOS_REPRINT_ANY();
   else if(IPC_EVENT_ID == d_IPC_CMD_PHQR_LOGIN)
       inPHQR_Login();    	
   else if (IPC_EVENT_ID == d_IPC_CMD_PHQR_SALE || inTranType == 60)
   {
   	   inPHQR_GenerateQR();
       if (fGetECRTransactionFlg())
           inMultiAP_ECRSendSuccessResponse();
   }
   else if (IPC_EVENT_ID == d_IPC_CMD_PHQR_RETRIEVE)
   		inCTOS_PHQRRetrieve();

   vdDebug_LogPrintf("testlang12  return");
   return d_OK;
}
