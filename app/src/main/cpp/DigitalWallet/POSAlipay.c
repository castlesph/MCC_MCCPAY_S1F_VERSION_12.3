...
int inCTOS_AlipayQRRequest(void)
{
	int inRet = d_NO;
	unsigned char bTC2ndbyte = 0,
		bTC3rdbyte = 0;

	vdDebug_LogPrintf("--inCTOS_AlipayQRRequest--");
	...
	//inIITRead(ALIPAY_ISSUER_INDEX); // Read IIT for trans title header -- sidumili
	...
	srTransRec.HDTid = ALIPAY_HOST_INDEX;
	...
	inRet = inCTOS_AlipayQRRequestFlowProcess();
...
int inCTOS_AlipayQRRequestFlowProcess(void)
{
	...
	vdDebug_LogPrintf("inCTOS_AlipayQRRequestFlowProcess START");
	...
	if (iGetQRPresentedType() == MPQR_TYPE) 
	{
        ...
        while(1)
        {
    		...
    		if (inRet != d_OK) {
    			...
    			if (!srTransRec.fXMLTradeStateOK && strlen(srTransRec.szTradeState) > 0 &&
    				(strcmp(srTransRec.szTradeState, TRADE_STATE_NOTPAY) == 0)) 
    			{
					if(fExit == VS_FALSE)
						continue;
    ...
    vdDebug_LogPrintf("inCTOS_AlipayQRRequestFlowProcess inMultiAP_ECRSendSuccessResponse");
	inMultiAP_ECRSendSuccessResponse();
	...
	vdDebug_LogPrintf("inCTOS_AlipayQRRequestFlowProcess END");

    return d_OK;
}

int inCTOS_AlipayQRRetrieve(void)
{
	int inRet = d_NO;
	unsigned char bTC2ndbyte = 0,
		bTC3rdbyte = 0;

	vdDebug_LogPrintf("--inCTOS_AlipayQRRetrieve--");
	...
	inRet = inCTOS_AlipayQRRetrieveFlowProcess();
...
int inCTOS_AlipayQRRetrieveFlowProcess(void)
{
    int inRet = d_NO;
	int inEVENT_ID = 0;
	BOOL fExist = FALSE;
	...
	srRetrieveRec.fRetrieveInList = FALSE;
	...
	vdDebug_LogPrintf("-->>Alipay Search for TransactionID");
