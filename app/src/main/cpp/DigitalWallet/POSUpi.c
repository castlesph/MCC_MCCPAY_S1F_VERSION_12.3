...
#include "..\DigitalWallet\POSUpi.h"
...
int inCTOS_UpiQRRequest(void)
{
	int inRet = d_NO;
	unsigned char bTC2ndbyte = 0,
		bTC3rdbyte = 0;

	vdDebug_LogPrintf("--inCTOS_UpiQRRequest--");
	...
	//inIITRead(UPI_ISSUER_INDEX); // Read IIT for trans title header -- sidumili
   ...
   if(fGetECRTransactionFlg() == TRUE)
   {
	...
	srTransRec.HDTid = UPI_HOST_INDEX;
	...
	inRet = inCTOS_UpiQRRequestFlowProcess();
...
int inCTOS_UpiQRRequestFlowProcess(void)
{
	...
	vdDebug_LogPrintf("inCTOS_UpiQRRequestFlowProcess start");
    ...
    vdDebug_LogPrintf("inCTOS_UpiQRRequestFlowProcess inMultiAP_ECRSendSuccessResponse");
	inMultiAP_ECRSendSuccessResponse();
	...
	vdDebug_LogPrintf("inCTOS_UpiQRRequestFlowProcess END");

    return d_OK;
}

int inCTOS_UpiQRRetrieve(void)
{
	int inRet = d_NO;
	unsigned char bTC2ndbyte = 0,
		bTC3rdbyte = 0;

	vdDebug_LogPrintf("--inCTOS_UpiQRRetrieve--");
	...
	inRet = inCTOS_UpiQRRetrieveFlowProcess();
...
int inCTOS_UpiQRRetrieveFlowProcess(void)
{
    int inRet = d_NO;
	int inEVENT_ID = 0;
	BOOL fExist = FALSE;
	...
	vdDebug_LogPrintf("-->>Upi Search for TransactionID");
