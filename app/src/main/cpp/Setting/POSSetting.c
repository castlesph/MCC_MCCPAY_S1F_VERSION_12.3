...
//FOR QRPAY
int vdDisplayTrxn(int inTranType, char *sztrxlogo)
{
    int inMenuNumRecs = 0,
            inCtr = 0,
            inKey = 0,
    ...
    vdDebug_LogPrintf("saturn inSaleType[%d]", inTranType);


    inMenuNumRecs = inReadDMTrx(inTranType);

    vdDebug_LogPrintf("saturn inMenuNumRecs[%d]", inMenuNumRecs);

    memset(szMenuItems, 0x00, sizeof(szMenuItems));
	//test
	strcpy(szMenuItems, sztrxlogo);
	//strcpy(szMenuItems, "szSQL1.bmp");	
    strcat(szMenuItems, "|");
    ...
    vdDebug_LogPrintf("saturn szMenuItems[%s]", szMenuItems);


	inKey = MenuTransactions(szMenuItems, szTransFunc);
    

    vdDebug_LogPrintf("saturn inKey[%d]", inKey);
    ...
    if(inKey == d_OK)
    {
        vdDebug_LogPrintf("saturn execute trans %s", szTransFunc);
		vdSetGlobalVariable();
