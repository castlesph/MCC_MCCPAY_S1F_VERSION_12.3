...
int inCTOS_CheckInitComm(int inCommType)
{
    ...
    vdDebug_LogPrintf("inCTOS_CheckInitComm.start [%d]", inCommType);
	...
	{
		vdDebug_LogPrintf("inCTOS_CheckInitComm.3");
	...
	{
		vdDebug_LogPrintf("inCTOS_CheckInitComm.4");
	
		result = srCommFuncPoint.inInitComm(&srTransRec,WIFI_MODE);
		vdDebug_LogPrintf("inCTOS_CheckInitComm result [%d]", result);
...
void vdCTOS_DispStatusMessage(char* szMsg)
{
	...
	#else
    vdDisplayMessageStatusBox(1, 8, szMsg, MSG_PLS_WAIT, MSG_TYPE_PROCESS);
	/*
    if(strcmp(szMsg,"PROCESSING")==0)
    CTOS_Delay(300);
    else
    CTOS_Delay(1000);	
    */
